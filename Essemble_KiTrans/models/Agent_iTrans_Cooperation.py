...
class iTransformer_agent(nn.Module):
    """
    Paper link: https://arxiv.org/abs/2310.06625
    """

    def __init__(self, configs,seq_len,pred_len):
        super(iTransformer_agent, self).__init__()
        self.task_name = configs.task_name
        self.seq_len = seq_len
        self.pred_len = pred_len
        
        # Embedding
        self.enc_embedding = DataEmbedding_inverted(self.seq_len, configs.d_model, configs.embed, configs.freq, configs.dropout)
        # Encoder
        ...
        if configs.enc_in == 862:
                self.compress_variable = nn.Linear(862,128)
                self.recover_variable = nn.Linear(128,862)
        elif configs.enc_in == 321:
                self.compress_variable = nn.Linear(321,32)
                self.recover_variable = nn.Linear(32,321)
    ...
    def forward(self, x_enc):
        ...
        if self.enc_in == 862 or self.enc_in == 321:
            enc_out = self.compress_variable(enc_out.permute(0, 2, 1)).permute(0, 2, 1)
        ...
        if self.enc_in == 862 or self.enc_in == 321:
            enc_out = self.recover_variable(enc_out.permute(0, 2, 1)).permute(0, 2, 1)
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.cal_collaboration_w = nn.Sequential(
            ...
            Rearrange('b s 1 -> b 1 s'),  
            nn.Linear(configs.seq_len, self.agent_num, bias=True),
            nn.Softmax(dim=-1)
        )

        self.projection = nn.Linear(configs.d_model, configs.pred_len, bias=True)
        agent_label_len = [configs.pred_len,configs.pred_len,configs.pred_len//2,6]
        for i in range(self.agent_num): 
            self.agents.append(iTransformer_agent(configs, configs.seq_len, agent_label_len[i%4]))
    ...
    def forward(self, x_enc,y, x_mark_enc=None, x_dec=None, x_mark_dec=None, mask=None,agent_labels=None,cal_agent_loss=False,finetune=False):
        agent_outputs = []
        ...
        for i, agent in enumerate(self.agents):
            first_layer_output = agent.initial_embed(x_enc)
        ...
        for layer in range(self.num_layers):  
            ...
            for i, agent in enumerate(self.agents):
                ...
                if layer  == self.num_layers-1:
                    ...
                    if agent.enc_in == 862 or agent.enc_in == 321:
                        layer_output = agent.recover_variable(layer_output.permute(0, 2, 1)).permute(0, 2, 1)    
        ...
        if cal_agent_loss:
            agent_loss = sum(self.criterion(final_outputs[i], agent_labels[i%4]) for i in range(self.agent_num))
            return  final_forecasting, agent_loss
        else:
            return final_forecasting,0
    def freeze_agents(self):
        for agent in self.agents:
            for param in agent.parameters():
                param.requires_grad = False
        # if cal_forecasting_loss:
        #     forecasting_loss = self.criterion(final_forecasting, y)
...
if __name__ == '__main__':
    ...
    B, T, C = 8, configs.seq_len, configs.enc_in  # batch size, sequence length, number of variables
    x_enc = torch.randn(B, T, C)
    y = torch.randn(B, configs.pred_len, C)  # 对应于 multi_grained_y_enc
